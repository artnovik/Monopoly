...
namespace Monopoly.Lobby_v2
{
	public class JoinGame : MonoBehaviour
	{
		...
		[SerializeField]
		private GameObject buttonConnectPrefab;
		...
		public void OnMatchList(bool success, string extendedInfo, List<MatchInfoSnapshot> matchList)
		{
			...
			{
				GameObject _buttonConnectGO = Instantiate(buttonConnectPrefab);
				_buttonConnectGO.transform.SetParent(buttonConnectParent);
				_buttonConnectGO.transform.localScale = Vector3.one;

				Connect _roomListItem = _buttonConnectGO.GetComponent<Connect>();
